...
def main():
    ...
    parser.add_argument(
        "--config", required=False, help="Path to configuration YAML file"
    )

    # Add data override arguments
    group = parser.add_mutually_exclusive_group()
    group.add_argument("--file", help="Override data source with a local file path")
    group.add_argument("--api-call", help="Override data source with an API URL")
    parser.add_argument(
        "--api-bearer-token",
    ...
    try:
        if args.file and (args.api_bearer_token or args.api_bearer_token_env):
            parser.error(
                "--api-bearer-token and --api-bearer-token-env require --api-call"
            )
