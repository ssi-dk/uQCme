...
class QCProcessor:
    ...
    def load_reference_data(self):
        ...
        try:
            ...
            self.logger.info(
                "✓ QC rules loaded: %d rules from %s", len(self.qc_rules), rules_path
            )
        ...
        try:
            ...
            self.logger.info(
                "✓ QC tests loaded: %d tests from %s", len(self.qc_tests), tests_path
            )
        except SchemaError as e:
        ...
        self.logger.info("✓ Mapping configuration loaded from %s", mapping_path)
    ...
    def prepare_run_data(
        self, data: pd.DataFrame, *, validate_schema: bool = True
    ) -> pd.DataFrame:
    ...
    def _apply_operator(
        self, value: Any, operator: str, threshold: Any, field_name: str = "unknown"
    ) -> bool:
        ...
        try:
            ...
            if (
                ...
                self.logger.debug(
                    f"Null/missing value for field '{field_name}', skipping comparison"
                )
                return False
    ...
    def _get_sample_attributes(self, sample: pd.Series) -> Dict[str, str]:
        ...
        attributes["assembly_type"] = sample.get("assembly_type", default_assembly)

        return attributes

    def _rule_matches_criteria(
        self, rule: pd.Series, sample_attributes: Dict[str, str]
    ) -> bool:
        ...
        sample_species = (
            str(sample_attributes["species"]).strip()
            if sample_attributes.get("species")
            else ""
        )
    ...
    def _determine_qc_outcomes(
        self, failed_rules: List[str], passed_rules: List[str]
    ) -> List[str]:
        ...
        for _, test in self.qc_tests.iterrows():
            ...
            has_passed_conditions = (
                pd.notna(passed_conditions)
                and isinstance(passed_conditions, str)
                and passed_conditions.strip()
            )

            # Check if failed_rule_conditions is specified
            has_failed_conditions = (
                pd.notna(failed_conditions)
                and isinstance(failed_conditions, str)
                and failed_conditions.strip()
            )
            ...
            if has_passed_conditions:
                ...
                any_evaluated = any(
                    rule in passed_rules or rule in failed_rules
    ...
    def _determine_qc_action(self, qc_outcomes: List[str]) -> str:
        ...
        if not qc_outcomes:
            ...
            pass_test = self.qc_tests[self.qc_tests["outcome_id"] == "PASS"]
        ...
        for outcome_id in qc_outcomes:
            test_match = self.qc_tests[self.qc_tests["outcome_id"] == outcome_id]
        ...
        try:
            ...
            if warnings_data:
                ...
                empty_df = pd.DataFrame(
                    columns=["warning_type", "warning_message", "timestamp"]
                )
                empty_df.to_csv(warnings_path, sep="\t", index=False)
                info_msg = f"✓ No warnings - empty file created at {warnings_path}"
