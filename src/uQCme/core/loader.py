...
def _extract_api_error_detail(response: Optional[requests.Response]) -> Optional[str]:
...
def _resolve_project_scope_request(
    api_url: str, custom_headers: Optional[Dict[str, str]] = None
...
def get_unique_columns_from_mapping(
    ...
    for section_data in mapping_config["Sections"].values():
        for field_config in section_data.values():
            ...
            is_unique = bool(data_config.get("unique") or report_config.get("unique"))
            if not is_unique:
                continue
...
def collect_duplicate_row_warnings(df: pd.DataFrame, columns: List[str]) -> List[str]:
...
def validate_unique_columns(
    df: pd.DataFrame, mapping_config: Optional[Dict[str, Any]]
) -> None:
    ...
    if errors:
        raise ValidationError("Unique column validation failed: " + " ".join(errors))


def validate_run_data_frame(
    df: pd.DataFrame, mapping_config: Optional[Dict[str, Any]] = None
) -> None:
...
def _load_api_payload(
    ...
    try:
        verify_ssl = True
        try:
            response = _send_request(verify=verify_ssl, request_headers=headers)
        except requests.exceptions.SSLError:
            warning_msg = (
                "SSL verification failed, retrying without SSL verification..."
            )
        ...
        raise DataLoadError(f"API request failed: {e}", debug_info=debug_info)
    except ValueError as e:
        raise DataLoadError(f"Invalid JSON response: {e}", debug_info=debug_info)
...
def load_data_from_api(
    ...
    data, _ = _load_api_payload(
        api_url, bearer_token=bearer_token, custom_headers=custom_headers
    )
