...
class QCRulesSchema(pa.DataFrameModel):
    ...
    assembly_type: Series[str] = pa.Field(description="Target assembly type or 'all'")
    software: Optional[Series[str]] = pa.Field(
        nullable=True, description="Software used to generate the metric"
    )
...
class QCTestsSchema(pa.DataFrameModel):
    ...
    outcome_name: Series[str] = pa.Field(description="Display name for the outcome")
    description: Series[str] = pa.Field(description="Description of the outcome")
...
class RunDataSchema(pa.DataFrameModel):
    ...
    sample_name: Optional[Series[str]] = pa.Field(
        nullable=True, description="Optional sample identifier"
    )
    species: Optional[Series[str]] = pa.Field(nullable=True, description="Species name")
