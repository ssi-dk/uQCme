...
class TestUQCmeOutcomes(unittest.TestCase):
    ...
    def test_example_pass_general_fail_species_kp(self):
        ...
        outcomes, failed = self._get_outcome_set("example_pass_general_fail_species_kp")
    ...
    def test_example_warn_review(self):
        ...
        for rule in ["PASS1", "PASS2", "PASS3", "PASS4", "PASS5"]:
            self.assertIn(
                rule, failed, f"Expected {rule} in failed rules for WARN sample"
            )
    ...
    def test_sample_count(self):
        ...
        self.assertEqual(
            len(self.results), 19, f"Expected 19 samples, got {len(self.results)}"
        )
    ...
    def test_no_empty_outcomes(self):
        ...
        for idx, row in self.results.iterrows():
            ...
            with self.subTest(sample=sample_name):
                ...
                self.assertNotEqual(
                    outcome, "", f"Sample {sample_name} has empty outcome"
                )
                self.assertIsInstance(
                    outcome, str, f"Sample {sample_name} outcome is not string"
                )
        ...
        for idx, row in self.results.iterrows():
            ...
            with self.subTest(sample=sample_name):
                if outcomes == {"PASS", "PASS_GENERAL"}:
                    ...
                    "FAIL" in outcomes
                    or "FAIL_GENERAL" in outcomes
                    or any(o.startswith("FAIL_") for o in outcomes)
                ):
...
class TestDataIntegrity(unittest.TestCase):
    ...
    def test_data_file_structure(self):
        ...
        for col in required_rule_columns:
            self.assertIn(
                col, rules.columns, f"Required column {col} missing from QC_rules.tsv"
            )
